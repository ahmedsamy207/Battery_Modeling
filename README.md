...
- **Mission dashboard** (`dashboard/mission.html`) — aircraft mission /
  ...
  Replays the exact Simscape mission exports `results/mission_x57.csv` /
  `results/mission_evtol.csv` generated by `matlab/Aircraft_Mission.m` (NASA
  X-57 style cruise mission and eVTOL stress mission on the scripted pack),
  plotted against pack voltage, current, power, SOC, altitude, range, C-rate
  and energy, with aircraft visuals, KPI/telemetry readouts, a phase table, and
  a mission timeline. Playback includes a Play/Pause/Reset control, a draggable
  timeline slider, and a **playback speed factor** (`1×`, `5×`, `10×`, `25×`,
  `50×`, `100×`) that only affects dynamic playback. If an export is missing,
  the page shows generation instructions instead of substitute data.
